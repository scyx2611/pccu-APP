...
async function run() {
  ...
  try {
    console.log('[1] login');
    await page.goto('https://ecampus.pccu.edu.tw/eCampus/default.aspx', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);

    const loginFrame = page.frames().find((frame) => frame.url().includes('ap1.pccu.edu.tw')) || page;
    ...
    if (await studentTab.count()) {
      ...
      await popupPage.waitForTimeout(4000);
      await popupPage.screenshot({ path: path.join(outDir, 'step-02-student-tab.png'), fullPage: true });
    }

    console.log('[5] try search');
    const searchBtn = popupPage.locator('#Search, input[type="submit"][value*="查詢"], input[type="button"][value*="查詢"], button:has-text("查詢")').first();
